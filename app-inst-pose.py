...
modelpose = YOLO('yolo26n-pose.pt')

IMAGE_SIZE_CHOICES = [320, 512, 640]
CUSTOM_CSS = (Path(__file__).parent / "makers.css").read_text()

@spaces.GPU
def predict_image(img, conf_threshold, iou_threshold, show_inst, show_pose, imgsz):
    if show_inst and show_pose:
        ...
        poseresults = modelpose.predict(source=img, save=False, show_boxes=False, show_labels=False, show_conf=False)
        segresults_image_bgr = results[0].plot()
        annotated_image_bgr = poseresults[0].plot(img=segresults_image_bgr, labels=False, boxes=False, conf=False)
    elif show_inst:
        results = model.predict(
            source=img,
            conf=conf_threshold,
        ...
        poseresults = modelpose.predict(source=img, save=False, show_boxes=False, show_labels=False, show_conf=False)
        annotated_image_bgr = poseresults[0].plot(labels=False, boxes=False, conf=False)
    else:
        annotated_image_bgr = np.asarray(img)
   
    annotated_image_rgb = annotated_image_bgr[..., ::-1]
    return Image.fromarray(annotated_image_rgb)

@spaces.GPU
def predict_video(video_path, conf_threshold, iou_threshold, show_inst, show_pose, imgsz):
    ...
    while True:
        ...
        if show_inst and show_pose:
            ...
            poseresults = modelpose.predict(source=frame, save=False, show_boxes=False, show_labels=False, show_conf=False)
            segresults_image_bgr = results[0].plot()
            annotated_image_bgr = poseresults[0].plot(img=segresults_image_bgr, labels=False, boxes=False, conf=False)
        elif show_inst:
            results = model.predict(
            ...
            annotated_image_bgr = np.asarray(frame)
...
@spaces.GPU
def predict_webcam(frame, conf_threshold, iou_threshold, show_inst, show_pose, imgsz):
    if frame is None:
        return None
    ...
    if isinstance(frame, np.ndarray):
        ...
        if show_inst and show_pose:
            ...
            poseresults = modelpose.predict(source=frame_bgr, save=False, show_boxes=False, show_labels=False, show_conf=False)
            segresults_image_bgr = results[0].plot()
            annotated_image_bgr = poseresults[0].plot(img=segresults_image_bgr, labels=False, boxes=False, conf=False)
        elif show_inst:
            results = model.predict(
            ...
            annotated_image_bgr = np.asarray(frame_bgr)
...
with gr.Blocks(title="Instance Segmentation & Pose Estimation Fusion by YOLOv26 🚀") as nyiapp:
    gr.Markdown(
        """
<div align="center">
    <h1>Instance Segmentation & Pose Estimation Fusion by YOLOv26 🚀 </h1>
</div>
YOLOv26 Fusion of Instance Segmentation & Pose Estimation Inference.
        ...
        with gr.TabItem("📷 Image"):
            with gr.Row():
                with gr.Column():
                    ...
                    img_pose = gr.Checkbox(value=True, label="Show Pose results")
            ...
            img_btn.click(
                predict_image,
                inputs=[img_input, img_conf, img_iou, img_inst, img_pose, img_size],
                outputs=img_output,
            )
            ...
            gr.Examples(
                examples=[
                    ...
                    ["sample3.jpg", 0.25, 0.3, True, False, 640],
                    ["sample4.jpg", 0.25, 0.3, True, False, 640],
                ],
                inputs=[img_input, img_conf, img_iou, img_inst, img_pose, img_size],
                outputs=img_output,
                fn=predict_image,
        ...
        with gr.TabItem("🎬 Video"):
            with gr.Row():
                with gr.Column():
                    ...
                    vid_pose = gr.Checkbox(value=True, label="Show Pose results")
            ...
            vid_btn.click(
                predict_video,
                inputs=[vid_input, vid_conf, vid_iou, vid_inst, vid_pose, vid_size],
                outputs=vid_output,
            )
        ...
        with gr.TabItem("📹 Webcam"):
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    webcam_pose = gr.Checkbox(value=True, label="Show Pose results")
            ...
            webcam_input.stream(
                predict_webcam,
                inputs=[webcam_input, webcam_conf, webcam_iou, webcam_inst, webcam_pose, webcam_size],
